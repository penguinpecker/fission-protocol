...
pub mod FissionAMM {
    ...
    struct Storage {
        ...
        reserve_sy: Map::<u32, u256>,
        reserve_pt: Map::<u32, u256>,
        lp_supply: Map::<u32, u256>,
        lp_balances: Map::<(u32, ContractAddress), u256>,
        fees: Map::<u32, u256>,
    }
    ...
    fn add_liquidity(ref self: ContractState, market: u32, sy_amt: u256, pt_amt: u256) -> u256 {
        ...
        let r_sy = self.reserve_sy.read(market);
        let r_pt = self.reserve_pt.read(market);

        let lp = if total_lp == 0 {
            (sy_amt + pt_amt) / 2
        } else {
            let a = (sy_amt * total_lp) / r_sy;
            let b = (pt_amt * total_lp) / r_pt;
            if a < b { a } else { b }
        };

        self.reserve_sy.write(market, r_sy + sy_amt);
        self.reserve_pt.write(market, r_pt + pt_amt);
        self.lp_supply.write(market, total_lp + lp);
        self.lp_balances.write((market, caller), self.lp_balances.read((market, caller)) + lp);
        self.emit(AddLiq { user: caller, market, sy: sy_amt, pt: pt_amt, lp });
        lp
    }
    ...
    fn remove_liquidity(ref self: ContractState, market: u32, lp_amt: u256) -> (u256, u256) {
        ...
        let caller = get_caller_address();
        let user_lp = self.lp_balances.read((market, caller));
        assert(user_lp >= lp_amt, 'AMM: low lp');

        let total_lp = self.lp_supply.read(market);
        let r_sy = self.reserve_sy.read(market);
        let r_pt = self.reserve_pt.read(market);

        let sy_out = (lp_amt * r_sy) / total_lp;
        let pt_out = (lp_amt * r_pt) / total_lp;

        self.reserve_sy.write(market, r_sy - sy_out);
        self.reserve_pt.write(market, r_pt - pt_out);
        ...
        self.emit(RemoveLiq { user: caller, market, lp: lp_amt });
        (sy_out, pt_out)
    }

    /// Swap exact PT for SY (sell PT)
    #[abi(embed_v0)]
    fn swap_pt_for_sy(ref self: ContractState, market: u32, pt_in: u256, min_sy_out: u256) -> u256 {
        ...
        let fee = (sy_out_raw * FEE_BPS) / BPS;
        let sy_out = sy_out_raw - fee;
        ...
        self.reserve_pt.write(market, r_pt + pt_in);
        self.reserve_sy.write(market, r_sy - sy_out);
        self.fees.write(market, self.fees.read(market) + fee);

        self.emit(Swap { user: caller, market, sy_delta: sy_out, pt_delta: pt_in });
        sy_out
    }

    /// Swap exact SY for PT (buy PT = lock fixed yield)
    #[abi(embed_v0)]
    fn swap_sy_for_pt(ref self: ContractState, market: u32, sy_in: u256, min_pt_out: u256) -> u256 {
        ...
        let fee = (sy_in * FEE_BPS) / BPS;
        let sy_after_fee = sy_in - fee;
        ...
        self.reserve_sy.write(market, r_sy + sy_in);
        self.reserve_pt.write(market, r_pt - pt_out);
        self.fees.write(market, self.fees.read(market) + fee);

        self.emit(Swap { user: caller, market, sy_delta: sy_in, pt_delta: pt_out });
        pt_out
    }
    ...
    #[abi(embed_v0)]
    fn get_lp_supply(self: @ContractState, market: u32) -> u256 {
        self.lp_supply.read(market)
    }
    ...
    #[abi(embed_v0)]
    fn get_fees(self: @ContractState, market: u32) -> u256 {
        self.fees.read(market)
    }
